<template>
  ...
  <div class="disclaimer">Interview_Assistant 的回答仅供参考。</div>
...
onMounted(() => {
  ...
  on('ocr-text', (text) => {
    solution.setOCRText(text || '')
    ui.showToast('截图文字识别完成，可编辑后发送', 'success', 1500)
  })

  on('start-solving', (isFollowUp) => {
    lastIsFollowUp = !!isFollowUp
    proceedWithSolve(lastIsFollowUp)
  })

  function proceedWithSolve(isFollowUp = false) {
    ...
    if ((isFollowUp || s.keepContext) && solution.history.length > 0 && solution.activeHistoryIndex === 0) {
  ...
  on('toggle-visibility', (isVisible) => {
    ui.flash('toggle')
    ui.isStealthMode = !isVisible
    ui.showToast(isVisible ? '窗口已显示' : '窗口已隐藏', isVisible ? 'success' : 'info')
  })
  ...
  on('solution-stream-start', () => {
    ui.hasStarted = true
    solution.handleStreamStart(settingsStore.settings.keepContext, lastIsFollowUp)
  })
